...
def test_classify_triplet_topology_for_all_three_topologies():
    species_triplet = ("A", "B", "C")

    assert classify_triplet_topology(_tree("((A:1,B:1):1,C:1);"), species_triplet) == "con"
    assert classify_triplet_topology(_tree("((B:1,C:1):1,A:1);"), species_triplet) == "dis1"
    assert classify_triplet_topology(_tree("((A:1,C:1):1,B:1);"), species_triplet) == "dis2"
...
def test_run_triplet_pipeline_keeps_species_concordant_and_ranks_frequencies():
    species_triplet = ("A", "B", "C")
    ...
    assert result.concordant_diff is True
...
"""
    ...
    assert "concordant_diff" in out
    assert "A,B,C" in out


def test_write_pipeline_results_adds_diff_tag(tmp_path):
    species_triplet = ("A", "B", "C")
...
def test_write_pipeline_results_marks_discordant_highest_freq(tmp_path):
    ...
    out = output_file.read_text()
    assert "((B,C),A) [highest freq]" in out
